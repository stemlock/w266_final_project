...
from transformers import DistilBertForMaskedLM, DistilBertModel, DistilBertTokenizerFast, DistilBertForSequenceClassification 
...
class BiasEvaluator():
    ...
    def evaluate_intrasentence(self):
        ...
        model = utils.DistilBertForSequenceClassification(num_labels)
        ...
        for batch_num, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            for idx, prediction in enumerate(predictions[:, 0][:, 0]):
